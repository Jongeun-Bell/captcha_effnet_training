...
ArXiv ImageNet 일부를 활용해 데이터를 재구성하였으며, PyTorch, MLflow, EfficientNet을 활용해 모델 학습–평가–추론–시각화(t-SNE) 파이프라인을 구축했다.
...
├─ inference.py            # 단일 이미지 분류 + t-SNE 임베딩 추출
...
```
✔ 세부 클래스 → 대그룹 매핑 방식
프로젝트는 “목적에 맞게 단순화된 2-Class CAPTCHA 모델”을 사용하며, 이 매핑 로직은 captcha_dataset.py 내부에 존재한다. 
...
- 학습된 모델을 기반으로:
  - 단일 이미지 분류 (Animal/Object 확률 출력)
  - EfficientNet 특징 벡터(1280차원) 추출
  - t-SNE 임베딩 생성
  - 결과 시각화(tsne_visualization_captcha.png)
...
#### tsne_visualization.py — EfficientNet 실제 학습 기반 버전
- 특징
